...
-- Вывод уведомлений в чат (видимо только вам)
local function printToLocalChat(text)
    pcall(function()
        game:GetService("StarterGui"):SetCore("ChatMakeSystemMessage", {
            Text = "[DELTA SYSTEM]: " .. text,
...
-- Отключение коллизии блоков во время авто-фарма
...
-- Цикл автоматического фарма сокровищ
task.spawn(function()
    while true do
...
-- ГЛОБАЛЬНАЯ ФУНКЦИЯ ДЛЯ ВВОДА КОМАНД ЧЕРЕЗ КОНСОЛЬ DELTA
_G.DeltaConsole = function(inputMessage)
    if not inputMessage or inputMessage == "" then return end
    
    -- Активация ключа
    if inputMessage == "key_170xe3" then
        menuUnlocked = true
        printToLocalChat("Доступ Разрешен! Команды в консоли Delta разблокированы.")
        return
    end

    if not menuUnlocked then 
        printToLocalChat("Ошибка: Сначала введите команду key_170xe3")
        return 
    end

    -- Обработка аргументов
    local args = string.split(inputMessage, "_")
    ...
    if cmd == "farm" then
        ...
        printToLocalChat(farmActive and "Авто-фарм запущен!" or "Авто-фарм остановлен.")
        ...
        pcall(function()
            ...
            if targetFolder then
                ...
                if writefile and #StolenData > 0 then
                    ...
                    printToLocalChat("Ошибка! Инжектор не поддерживает запись файлов.")
                end
            else
...
printToLocalChat("Скрипт загружен! Для активации введите в консоль Delta: _G.DeltaConsole('key_170xe3')")
